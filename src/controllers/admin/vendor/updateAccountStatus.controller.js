...
const updateAccountStatus = asyncHandler (async (req, res) => {
    console.log("updateAccountStatus working", req.body);
    let session;
    ...
    try {
        ...
        if (fieldValidator(verified) ) throw new ApiError(statusCodeObject.HTTP_STATUS_BAD_REQUEST, errorAndSuccessCodeConfiguration.HTTP_STATUS_BAD_REQUEST, CommonMessage.ERROR_FIELD_REQUIRED);
        ...
        const respValue = await UserModel.updateOne({
            ...
            $set: {
                accountApprovedTime: new Date().getTime(),
                verified
            }
        }, {
            session: session
